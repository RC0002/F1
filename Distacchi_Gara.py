...
def execute(nameRace,yearRace,drivers,typeOfSession,secondiDaConsiderare, isTest):
    ...
    leader_laps = race.laps[race.laps['Position'] == 1][['LapNumber', 'Driver' ,'LapTime', 'IsAccurate']]
    leader_laps = leader_laps.sort_values(by='LapNumber')
    #st.write(leader_laps.iloc[-1])
    leader_laps = race.laps.pick_drivers(leader_laps.iloc[-1]['Driver'])#.pick_accurate()
    #st.write(leader_laps)
    ...
    for d in drivers:
        ...
        if (laps.LapNumber.values.size > 0):
            ...
            l['LapTime'] = l['LapTime'].dt.total_seconds()
            l = pd.merge(l, leader_laps, on='LapNumber', suffixes=('', '_Leader'))
            l['LapTime_Leader'] = l['LapTime_Leader'].dt.total_seconds()
            l['GapToLeader'] = l['LapTime'] - l['LapTime_Leader']
            l['GapToLeader'] = l['GapToLeader'].fillna(0)
            l['Cumulative_Gap'] = l['GapToLeader'].cumsum()
            l['LeaderLap'] = leader_laps['LapTime']
            #st.write(laps.head())
            #st.write(leader_laps.head())
            #st.write(l[['Driver', 'GapToLeader', 'Cumulative_Gap', 'LapTime_Leader', 'LapTime']])
            chart_dataSoft = pd.DataFrame({
                "Driver": d,
                "LapNumber": l['LapNumber'],
                "Cumulative_Gap": l['Cumulative_Gap']
            })
            ...
            #if (l['GapToLeader'].max() > maxNumberPos):
                #maxNumberPos = l['Position'].max()

            if (l['LapNumber'].max() > maxNLap):
                maxNLap = l['LapNumber'].max()
            if (l['Cumulative_Gap'].max() > maxDIst):
                maxDIst = l['Cumulative_Gap'].max()
    ...
    if data_list is not None:
        chartS = alt.Chart(data_list).mark_line().encode(
            ...
            y=alt.Y(
                'Cumulative_Gap:Q',
                scale=alt.Scale(domain=[maxDIst, 0])  # Imposta la scala per l'asse Y
            ),
            color=alt.Color(
            ...
            tooltip=['Driver', 'LapNumber', 'Cumulative_Gap']  # Aggiungi tooltip interattivi
        ).properties(
        ...
        chartSPoints = alt.Chart(data_list).mark_circle().encode(
            ...
            tooltip=['Driver', 'LapNumber', 'Cumulative_Gap']
