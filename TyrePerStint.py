...
def execute(nameRace,yearRace,drivers,typeOfSession,secondiDaConsiderare, isTest):
    ...
    for d in drivers:
        ...
        if (laps.LapNumber.values.size > 0):
            ...
            for i in range(0,int(numStint)):
                ...
                if( not l.empty):
                    #st.write(l['Compound'].iloc[0])
                    if(l['Compound'].iloc[0] == 'MEDIUM'):
                        clr = "#FFFF00"
                    elif(l['Compound'].iloc[0] == 'SOFT'):
                        clr = "#FF0000"
                    elif (l['Compound'].iloc[0] == 'HARD'):
                        clr = "#FFFFFF"
                    elif (l['Compound'].iloc[0] == 'WET'):
                        clr = "#0000FF"
                    else:
                        clr = "#00FF00"
                else:
                ...
                if (driverNumber > maxNumberPos):
                    maxNumberPos = driverNumber
    ...
    if data_list is not None:
        chartS = alt.Chart(data_list).mark_line().encode(
            ...
            tooltip=['Driver', 'RaceLapNumber', 'TyreLife', 'Compound']  # Aggiungi tooltip interattivi
        ).properties(
        ...
        chartSPoints = alt.Chart(data_list).mark_circle().encode(
            ...
            tooltip=['Driver', 'RaceLapNumber', 'TyreLife', 'Compound']
